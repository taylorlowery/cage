...
void test_http_client_request_get(void) {
    HTTPResponse *resp =
        http_request(HTTP_GET, "example.com", "80", NULL, NULL, 0, NULL, stdout, stderr);
...
void test_https_client_request_get(void) {
    HTTPResponse *resp =
        https_request(HTTP_GET, "example.com", "443", "/", NULL, 0, NULL, stdout, stderr);
...
void test_post_to_invalid_endpoint_returns_error_status(void) {
    char *body = "{\"test\":\"data\"}";
    HTTPResponse *resp =
        http_request(HTTP_POST, "example.com", "80", "/post", NULL, 0, body, stdout, stderr);
...
void test_http_client_request_post(void) {
    ...
    char *body = "{\"test\": \"data\"}";
    HTTPResponse *resp =
        http_request(HTTP_POST, "httpbin.org", "80", "/post", headers, 3, body, stdout, stderr);
    ...
    if (resp) {
        ...
        TEST_ASSERT_NOT_NULL_MESSAGE(strcasestr(resp->body, "\"x-api-key\": \"deadbeef\""),
                                     "x-api-key header not found in response body");
        TEST_ASSERT_NOT_NULL_MESSAGE(strcasestr(resp->body, "\"x-tenant-context\": \"1337\""),
                                     "x-tenant-context header not found in response body");
        TEST_ASSERT_NOT_NULL_MESSAGE(strcasestr(resp->body, "\"test\": \"data\""),
                                     "test header not found in response body");
        free_http_response(resp);
    }
}
...
void test_https_client_request_post(void) {
    ...
    char *body = "{\"test\": \"data\"}";
    HTTPResponse *resp =
        https_request(HTTP_POST, "httpbin.org", "443", "/post", headers, 3, body, stdout, stderr);
